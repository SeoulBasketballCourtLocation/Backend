...
def build_base():
    try:
        ...
        # docker build
        subprocess.call('docker build -t basketball:base -f Dockerfile.base .', shell=True)
    finally:
...
def build_local():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t basketball:local -f Dockerfile.local .', shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
...
def build_dev():
    try:
        ...
        # docker build
        subprocess.call('docker build -t basketball:dev -f Dockerfile.dev .', shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
...
def build_production():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t basketball:production -f Dockerfile.production .',
                        shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
